...
- Response: `{ message: "success", data: [ { _id, username, role, createdAt, updatedAt }, ... ] }`
- Notes: Password excluded from response
...
- Guards: `AdminGuard`
- Body: `UpdateUserDto` -> `{ "username?": string, "password?": string, "role?": string }`
- Response: `{ message: "User updated successfully", data: { ...updatedUser } }`
- Notes: Password will be hashed if provided
...
- Guards: `AdminGuard`
- Response: `{ message: "User deleted successfully", data: { id } }`
- Error: 401 if user not found
